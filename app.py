...
async def on_ready():
    ...
    for guild in client.guilds:
        create_directory_in_parent(guild.name)
...
def get_guild_folder(guild : discord.Guild):
    ...
    if "/" in str(path.parent):
        newpath = str(path.parent.absolute()) + "/" + guild.name
    else:
        newpath = str(path.parent.absolute()) + "\\" + guild.name
    return newpath
...
def create_user_directory(message : discord.Message):
    path = Path(__file__)

    newpath = ""
    if "/" in str(path.parent):
        newpath = str(path.parent.absolute()) + "/" + message.guild.name + "/" + message.author.name
    else:
        newpath = str(path.parent.absolute()) + "\\" + message.guild.name + "\\" + message.author.name
    if not os.path.exists(newpath):
        os.makedirs(newpath)
...
def add_XP(user : discord.Member):
    ...
    newpath = ""
    if "/" in str(path.parent):
        newpath = str(path.parent.absolute()) + "/" + user.guild.name + "/" + user.name
    else:
        newpath = str(path.parent.absolute()) + "\\" + user.guild.name + "\\" + user.name
    if not os.path.exists(newpath):
        os.makedirs(newpath)
    print(combine_paths(newpath, "level.txt"))
    with open(combine_paths(newpath, "level.txt"), 'r+') as file:
        # Write content to the file
...
def set_XP(user : discord.Member, new_XP : int):
    ...
    print(combine_paths(newpath, "level.txt"))
    if not os.path.exists(combine_paths(newpath, "level.txt")):
        with open(combine_paths(newpath, "level.txt"), "x+") as file:
            file.write("0")
    with open(combine_paths(newpath, "level.txt"), 'r+') as file:
        # Write content to the file
        if not file.readable():
...
def read_level(user : discord.Member):
    ...
    if os.path.exists(combine_paths(newpath, "level.txt")):
        with open(combine_paths(newpath, "level.txt"), "r+") as file:
            # Write content to the file
...
async def on_message(msg : discord.Message):
    try:
        ...
        newpath = ""
        if "/" in str(path.parent):
            newpath = str(path.parent.absolute()) + "/" + msg.guild.name + "/" + msg.author.name
        else:
            newpath = str(path.parent.absolute()) + "\\" + msg.guild.name + "\\" + msg.author.name
        if not os.path.exists(newpath):
            create_user_directory(msg) # why cant message.author always return a member and not a user
        else:
            if "/" in str(path.parent):
            ...
            # Level roles
            path = Path(__file__)
            newpath = str(path.parent.absolute()) + "/" + msg.guild.name + "/level_roles.txt"
...
async def on_guild_remove(guild : discord.Guild):
    if guild.name != None:
        # Code for getting a guild's folder
        path = Path(__file__)
...
async def add_level_role(interaction : discord.Interaction, level : int, role : discord.Role):
    if interaction.user.guild_permissions.manage_guild:
        path = Path(__file__)
        newpath = str(path.parent.absolute()) + "/" + interaction.guild.name + "/level_roles.txt"
    ...
    if globally:
        for guild in client.guilds:
            if interaction.user in guild.members:
                ...
                newpath = ""
                if "/" in str(path.parent):
                    newpath = str(path.parent.absolute()) + "/" + guild.name + "/" + interaction.user.name
                else:
                    newpath = str(path.parent.absolute()) + "\\" + guild.name + "\\" + interaction.user.name
...
async def set_level(interaction : discord.Interaction, member : discord.Member, new_xp : int):
    if not interaction.user.guild_permissions.manage_guild:
        await interaction.response.send_message(embeds.insufficient_permissions(Permissions.MANAGE_SERVER))
        return
